    ...
    <div className="bg-white rounded-lg shadow-md p-6" style={{ backgroundColor: 'var(--bg-card)' }}>
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
          ...
          <span className="text-lg font-bold text-blue-900">₹{totalBudget.toFixed(2)}</span>
        </div>

        <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
          <span className="text-sm font-medium text-red-800">Total Monthly Spending</span>
          <span className="text-lg font-bold text-red-900">₹{totalSpending.toFixed(2)}</span>
        </div>

        <div className={`flex justify-between items-center p-3 rounded-lg ${remainingBudget >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
          <span className={`text-sm font-medium ${remainingBudget >= 0 ? 'text-green-800' : 'text-red-800'}`}>
            Remaining Budget
          </span>
          <span className={`text-lg font-bold ${remainingBudget >= 0 ? 'text-green-900' : 'text-red-900'}`}>
